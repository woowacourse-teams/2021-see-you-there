...
public enum AuthExceptionSet implements CustomException {
    ...
    KAKAO_USER_INFO("카카오에서 정보를 가져오는 데 실패했습니다.", HttpStatus.INTERNAL_SERVER_ERROR.value()),
    NAVER_CALLBACK("네이버와 인증시도 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR.value()),
    NAVER_USER_INFO("네이버에서 정보를 가져오는 데 실패했습니다.", HttpStatus.INTERNAL_SERVER_ERROR.value());
